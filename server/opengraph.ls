@include = ->
  ...
  @getOpenGraph = (hash, code) ->
    console.log 'Stepped into getOpenGraph.'
    item = hash[code]
    ret = 
      og_title: ""
      og_url: ""
    ...
    ret.og_description = "【"+item[4]+" > " + item[5] + " > " + item[6] + " > " + item[3] + "】的年度預算為" + CurrencyConvert(item[1],0,true) + ", 相當於" + (CurrencyConvert item[1],(Math.random!%CurrencyData.length),true) + ", 也等於" + (CurrencyConvert item[1],parseInt(Math.random!%CurrencyData.length),true)
    return ret;
